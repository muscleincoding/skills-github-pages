...
def create_index(filenames, index, file_titles):
    ...
    """
    if len(filenames) == 0:
        for key in file_titles:
            filenames.append(key)
    # 列表非空
    else:
        for filename in filenames:
            file = open(filename, 'r')
            file_title = file.readline().strip()
            # print(file_title)
            file_titles[filename] = file_title
            with open(filename, 'r') as file: # 为什么这里要再打开一次？因为光标
                for line in file:
...
def search(index, query):
    ...
    if term_list[0] in index:
        initial_list = index[term_list[0]]
        outcome_list = initial_list
        for term in term_list[1:]:
            if term not in index:
                outcome_list = []
                return outcome_list
            else:
                outcome_list = common(initial_list, index[term])
    else:
        outcome_list = []
    return outcome_list
    """
